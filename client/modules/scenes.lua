if not Citizen then
  ...
  Scenes.Synchronised = {
    ...
    PedConfig = function(ped,scene,animDict,animName,blendIn,blendOut,duration,flag,speed,unk1)
      if not ped      then print("[tools] Scenes.Synchronised.PedConfig needs a ped as the first argument.") return false end
      if not scene    then print("[tools] Scenes.Synchronised.PedConfig needs a scene as the second argument.") return false end
      if not animDict then print("[tools] Scenes.Synchronised.PedConfig needs an animDict as the third argument.") return false end
      if not animName then print("[tools] Scenes.Synchronised.PedConfig needs an animName as the fourth argument.") return false end
    ...
    EntityConfig = function(entity,scene,animDict,animName,blendIn,blendOut,flags)
      if not entity   then print("[tools] Scenes.Synchronised.EntityConfig needs a entity as the first argument.") return false end
      if not scene    then print("[tools] Scenes.Synchronised.EntityConfig needs a scene as the second argument.") return false end
      if not animDict then print("[tools] Scenes.Synchronised.EntityConfig needs an animDict as the third argument.") return false end
      if not animName then print("[tools] Scenes.Synchronised.EntityConfig needs an animName as the fourth argument.") return false end
  ...
  exports('SynchronisedScene', function() 
